...
create or replace index SPRING_SESSION_IX2
    on forum_test.spring_session (EXPIRY_TIME);

create or replace index SPRING_SESSION_IX3
    on forum_test.spring_session (PRINCIPAL_NAME);
...
(
    ...
    constraint SPRING_SESSION_ATTRIBUTES_FK
        foreign key (SESSION_PRIMARY_ID) references forum_test.spring_session (PRIMARY_ID)
            on delete cascade
)
    row_format = DYNAMIC;

create or replace index SPRING_SESSION_ATTRIBUTES_IX1
    on forum_test.spring_session_attributes (SESSION_PRIMARY_ID);
...
(
    ...
    constraint FKeatd60h39ra63bdk44w8vfvk2
        foreign key (category_id) references forum_test.categories (id),
    constraint FKqwy1e63idnvjerwvc47tq3k5
        foreign key (user_id) references forum_test.users (id)
);
...
(
    ...
    constraint FK7h839m3lkvhbyv3bcdv7sm4fj
        foreign key (parent_comment_id) references forum_test.comments (id),
    constraint FKja5i1vdiw6jmv6pno39qlr6od
        foreign key (user_id) references forum_test.users (id),
    constraint FKrcbvcroe2c5uu8ij99lb7vcfd
        foreign key (post_id) references forum_test.posts (id)
);
...
(
    ...
    constraint FKeouwh57y9blek9aa4mulekcui
        foreign key (id) references forum_test.posts (id),
    constraint FKm6cfovkyqvu5rlm6ahdx3eavj
        foreign key (tag_id) references forum_test.tags (id)
);
...
(
    ...
    constraint FKq5kuha6esfmijvmn2jcsap0bs
        foreign key (role_Id) references forum_test.roles (id)
);
